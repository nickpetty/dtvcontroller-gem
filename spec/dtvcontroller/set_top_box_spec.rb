...
module Dtvcontroller
  describe SetTopBox do
    ...
    # Typical set top box object
    let(:set_top_box) { Dtvcontroller::SetTopBox.new("192.168.1.100") }
    # Special IP address 1.2.3.4 triggers the Webmock timeout
    let(:stb_timeout) { Dtvcontroller::SetTopBox.new("1.2.3.4") }
    # Special IP address 4.3.2.1 triggers the Webmock runtime error
    let(:stb_runtime) { Dtvcontroller::SetTopBox.new("4.3.2.1") }
    ...
    describe "#system_info" do
      ...
      it "raises a RuntimeError on a timeout" do
        expect { stb_timeout.system_info(:version) }.to raise_error(RuntimeError)
      end

      it "raises a RuntimeError on a failed connection" do
        expect { stb_runtime.system_info(:version) }.to raise_error(RuntimeError)
      end
    end
    ...
    describe "#tune_to_channel" do
      ...
      it "raises a RuntimeError on a timeout" do
        expect { stb_timeout.tune_to_channel(7) }.to raise_error(RuntimeError)
      end

      it "raises a RuntimeError on a failed connection" do
        expect { stb_runtime.tune_to_channel(7) }.to raise_error(RuntimeError)
      end
    end
    ...
    describe "#get_currently_playing" do
      ...
      it "returns major and callsign and title and episode title for a tv show" do
        ...
        stb = Dtvcontroller::SetTopBox.new("192.168.1.101")

        expect(stb.currently_playing).to eq(tv_response)
      end

      # Use 192.168.1.102 for a movie
      it "returns major and callsign and title for a movie" do
        movie_response = "501: HBOeHD - Good Burger"
        stb = Dtvcontroller::SetTopBox.new("192.168.1.102")

        expect(stb.currently_playing).to eq(movie_response)
      end

      it "raises a RuntimeError on a timeout" do
        expect { stb_timeout.currently_playing }.to raise_error(RuntimeError)
      end

      it "raises a RuntimeError on a failed connection" do
        expect { stb_runtime.currently_playing }.to raise_error(RuntimeError)
      end
    end
    ...
    describe "#send_key" do
      ...
      it "raises a RuntimeError on a timeout" do
        expect { stb_timeout.send_key(:dash) }.to raise_error(RuntimeError)
      end

      it "raises a RuntimeError on a failed connection" do
        expect { stb_runtime.send_key(:dash) }.to raise_error(RuntimeError)
      end
    end
